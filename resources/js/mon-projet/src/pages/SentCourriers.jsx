...
export default function SentCourriers() {
  ...
  const loadData = async (params = {}) => {
    try {
      ...
      setError('Erreur lors du chargement des courriers envoyés.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
           ...
           <div>
             <h1 className="text-lg font-bold text-slate-900">Courriers Envoyés</h1>
             <p className="text-xs text-slate-500">Suivi des flux sortants.</p>
           </div>
        </div>
